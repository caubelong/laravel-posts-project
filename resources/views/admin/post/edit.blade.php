...
@section('content')
    <div id="main-content">
        ...
        <div class="main-create_posts">
            ...
            @endif
            <form action="{{route('posts.update',['post'=>$posts->p_id])}}" method="post" id="form-post"  enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label>Tiêu đề</label>
                    <textarea type="text" cols="2" rows="3" name="title"
                              class="form-control" placeholder="Tiêu đề...">{{$posts->title}}@if(old('title')){{old('title')}}@endif</textarea>
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control" name="category_id">
                        ...
                        @foreach($category as $cat)
                            <option value="{{$cat->chil_cate_id}}" @if($posts->category_id ===$cat->chil_cate_id)selected @endif>{{$cat->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="exampleFormControlTextarea1">Ảnh mô tả</label>
                    <img src="{{asset('img/'.$posts->cover_img)}}" style="width:300px;height: 200px" class="form-control-file">
                </div>
                <div class="form-group">
                    <label for="exampleFormControlTextarea1">Update ảnh mô tả</label>
                    <input type="file" name="cover_img" class="form-control-file">
                </div>
                <div class="form-group">
                    <label for="exampleFormControlSelect2">Mô tả tin tức</label>
                    <textarea type="text" name="description" rows="4" cols="50" class="form-control" placeholder="Mô tả ...">{{$posts->description}}@if(old('description')){{old('description')}}@endif</textarea>
                </div>
                <div class="form-group">
                    <label for="exampleFormControlTextarea1">Nội dung tin tức</label>
                    <textarea id="file-picker" name="body" rows="20" cols="50" class="form-control" rows="">{!! $posts->body !!}
                        @if(old('body')){{old('body')}}@endif</textarea>
                </div>
                <div class="form-group">
                    ...
                    <select class="form-control" name="option_show">
                        ...
                        <option value="1" @if($posts->option_show===1) selected @endif>Hiện</option>
                        <option value="0" @if($posts->option_show===0) selected @endif>Ẩn</option>
                    </select>
                </div>
                <div class="form-group">
                    <button type="submit" class="btn btn-primary float-right">Sửa tin</button>
                </div>
            </form>
        </div>
    </div>
